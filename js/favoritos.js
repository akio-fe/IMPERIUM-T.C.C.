...
const createCard = (item) => {
	...
	article.innerHTML = `
		...
		<div class="product__actions">
			...
			<button class="remove-button" data-produto-id="${item.produto.id}">Remover</button>
		</div>`;
	return article;
};
...
const renderFavoritos = (items) => {
	...
	setStatus(`${items.length} produto${items.length === 1 ? "" : "s"} favoritado${items.length === 1 ? "" : "s"}.`);
	items.forEach((item) => {
...
const removerFavorito = async (produtoId) => {
	...
	if (response.status === 401) {
		throw Object.assign(new Error("Faça login para gerenciar favoritos."), { status: 401 });
	}
	const payload = await response.json();
	if (!payload?.success) {
...
const handleRemoveClick = async (event) => {
	...
	try {
		...
		if (!containerEl.children.length) {
			...
			setStatus(`${containerEl.children.length} produto${containerEl.children.length === 1 ? "" : "s"} favoritado${containerEl.children.length === 1 ? "" : "s"}.`);
		}
	} catch (error) {
...
const initFavoritosPage = async () => {
	...
	try {
		...
		setStatus(error.message || "Não foi possível carregar seus favoritos.", true);
		if (error.status === 401) {
