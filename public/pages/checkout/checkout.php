...
try {
    ...
    // ===== Verificação do ID Token (Authorization: Bearer <token>) =====
    $headers = getallheaders();
    $idToken = null;
    if (isset($headers['Authorization'])) {
        $idToken = str_replace('Bearer ', '', $headers['Authorization']);
    ...
    try {
        /** @var UnencryptedToken $verifiedIdToken */
        $verifiedIdToken = $auth->verifyIdToken($idToken);
        $uid = $verifiedIdToken->claims()->get('sub'); // UID do usuário autenticado
    } catch (ExpiredToken $e) {
        http_response_code(401);
        echo json_encode(['success' => false, 'message' => 'Token expirado. Faça login novamente.']);
        exit;
    ...
    // ===== Leitura do corpo e validação dos campos obrigatórios =====
    $inputJSON = file_get_contents('php://input');
    $data = json_decode($inputJSON, true);
    ...
    // ===== UID no token deve corresponder ao UID do payload =====
    if ($uid !== $data['uid']) {
        http_response_code(401);
        echo json_encode(['success' => false, 'message' => 'UID do token não corresponde ao UID enviado.']);
        exit;
    ...
    // ===== Persistência em banco com validação de duplicidade =====
    try {
        $duplicateSql   = "SELECT COUNT(*) FROM usuario WHERE UsuCpf = ? OR UsuEmail = ?";
        $duplicateTypes = "ss";
        $duplicateValues = [$usuCpf, $usuEmail];
        ...
        $stmt_insert = $conn->prepare("INSERT INTO usuario (UsuUID, UsuEmail, UsuNome, UsuCpf, UsuTel, UsuDataNasc) VALUES (?, ?, ?, ?, ?, ?)");
        if (!$stmt_insert) {
            http_response_code(500);
            echo json_encode(['success' => false, 'message' => 'Erro ao preparar inserção do usuário: ' . $conn->error]);
            exit;
